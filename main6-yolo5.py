#!/usr/bin/env python3
# curve‑road safety ai — yolov5 metre‑based gap logic (full functionality kept)

from __future__ import annotations
from dataclasses import dataclass, field
import time, sys, math, cv2, numpy as np, torch
import warnings
...
class Config:
    ...
    light_classes: set[str] = field(default_factory=lambda: {"car", "motorbike", "motorcycle", "bicycle"})
    vehicle_widths: dict[str, float] = field(default_factory=lambda:{
        "bus":2.5,"truck":2.5,"car":1.8,"motorbike":0.8,"motorcycle":0.8,"bicycle":0.6})
...
def estimate_distance(px:int,cls:str,cfg:Config)->float:
    real=cfg.vehicle_widths.get(cls,1.8)
    return float('inf') if px==0 else (real*cfg.focal_length_px)/px
    ...
    def update(self,cents,boxes,names):
        ...
        if not cents:
            for oid in list(self.lost):
                self.lost[oid]+=1
                if self.lost[oid]>c.history_len:
                    for d in(self.cents,self.boxes,self.names,self.lost): d.pop(oid,None)
            return
        if not self.cents:
            for p,b,n in zip(cents,boxes,names): self._add(p,b,n); return
        oids=list(self.cents); D=self._pair(np.array([self.cents[i] for i in oids]),np.array(cents))
        used_r,used_c=set(),set()
        for r in D.min(1).argsort():
            cidx=int(D[r].argmin()); oid=oids[r]
            if r in used_r or cidx in used_c: continue
            if D[r,cidx]>c.centroid_match_px or bbox_iou(self.boxes[oid],boxes[cidx])<c.iou_match_thresh: continue
            self.cents[oid],self.boxes[oid],self.names[oid]=cents[cidx],boxes[cidx],names[cidx]
            self.lost[oid]=0; used_r.add(r); used_c.add(cidx)
        for r,oid in enumerate(oids):
            if r not in used_r:
                self.lost[oid]+=1
                if self.lost[oid]>c.history_len:
                    for d in(self.cents,self.boxes,self.names,self.lost): d.pop(oid,None)
        for cidx in range(len(cents)):
            if cidx not in used_c: self._add(cents[cidx],boxes[cidx],names[cidx])
    def _add(self,c,b,n):
...
class YoloV5Detector:
    ...
    def __call__(self,frame):
        ...
        for x1,y1,x2,y2,conf,cls in det:
            ...
            boxes.append([int(x1),int(y1),bw,bh])
            cents.append((cx,cy)); names.append(name)
        return cents,boxes,names

# ───────────────────── main loop ───────────────────────
def main(cfg:Config):
    cap=cv2.VideoCapture(1)
    if not cap.isOpened(): sys.exit("cannot open video or camera")
    cap.set(cv2.CAP_PROP_FRAME_WIDTH,cfg.frame_width)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT,cfg.frame_height)
    detect,tracker=YoloV5Detector(cfg),Tracker(cfg)
