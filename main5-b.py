...
# ---------- constants ----------
MODEL_PATH = "yolov8n.pt"           # ← put your yolov8 model here
FW, FH, CURVE_RAD = 1280, 720, 500
CURVE_CX, METERS_PER_PIXEL = FW // 2, 0       # set meters_per_pixel after calibration
...
MODEL_NAMES = yolo_model.names  # class id → name mapping

def detect(frame):
    results = yolo_model.predict(frame, imgsz=640, conf=0.5, iou=0.45, verbose=False)[0]
    boxes, centers, names = [], [], []
    for box, cls, conf in zip(results.boxes.xywh.cpu().numpy(),
                              results.boxes.cls.cpu().numpy(),
                              results.boxes.conf.cpu().numpy()):
        name = MODEL_NAMES[int(cls)]
        if name not in VEHICLE_CLASSES:
            continue
        cx, cy, bw, bh = map(int, box)
        x, y = int(cx - bw / 2), int(cy - bh / 2)
        boxes.append([x, y, bw, bh])
        centers.append((cx, cy))
        names.append(name)
    return centers, boxes, names
...
def main():
    ...
    while True:
        ...
        min_gap, pair = None, None
        for i in range(len(ids)):
            for j in range(i + 1, len(ids)):
                a, bpt = approaching[ids[i]], approaching[ids[j]]
                if (a[0] - CURVE_CX) * (bpt[0] - CURVE_CX) > 0: continue  # same side
                d = np.linalg.norm(np.subtract(a, bpt)) * METERS_PER_PIXEL
                if min_gap is None or d < min_gap: min_gap, pair = d, (a, bpt)
